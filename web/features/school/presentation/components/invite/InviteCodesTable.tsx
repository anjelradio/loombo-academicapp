import { Invite } from "@/features/school/domain/entities/invite";
import { formatBoliviaDateTime } from "@/lib/date-time";
import InviteCodeRowActions from "./InviteCodeRowActions";
...
export default function InviteCodesTable({ schoolId, items }: InviteCodesTableProps) {
  return (
    <>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-t border-[#243F63] bg-[#10233D]/90">
                ...
                <td className="px-6 py-5 text-sm text-[#C7D7EA]">
                  {formatBoliviaDateTime(item.expires_at)}
                </td>
                <td className="px-6 py-5">
                  <span
                ...
                <td className="px-6 py-5">
                  <InviteCodeRowActions schoolId={schoolId} inviteId={item.id} code={item.code} />
                </td>
              </tr>
            ))}
          </tbody>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div className="col-span-2">
                ...
                <p className="mt-1 text-[#E2ECF9]">{formatBoliviaDateTime(item.expires_at)}</p>
              </div>
            </div>
          </article>
        ))}
